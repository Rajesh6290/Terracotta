                ...
                {MenuItems?.map((menuItem) => (
                    <Fragment key={menuItem?._id}>
                        ...
                        {menuItem?.submenus && selectedSubMenu === menuItem?._id ? (
                            <div className="pl-4 py-1 flex flex-col gap-1" style={{ transition: "max-height 0.3s ease" }}>
                                {menuItem?.submenus.map((submenuItem) => (
                                    <div
                                        key={submenuItem?._id}
                                        className={`w-full flex items-center px-4 py-3 rounded-md text-gray-400 hover:text-primary hover:bg-primary/10 transition-all duration-150 ease-in-out cursor-pointer ${router.asPath === submenuItem.route
