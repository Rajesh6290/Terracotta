            ...
            <AccountLayout>
                ...
                >
                    <div className="flex flex-col gap-5 py-4">
                        ...
                        <div
                            className={`w-full  text-gray-800  ${showForm ? `` : `border p-4 rounded-lg`}`}

                        >
                            <p onClick={() => setShowForm(!showForm)} className={`w-full capitalize font-semibold tracking-wide ${showForm ? `hidden` : `flex cursor-pointer`}  items-center justify-between`}>
                                Add New Address
                                <MdExpandMore
                                    className={`text-2xl text-gray-800 common-transition ${showForm ? "" : " -rotate-90"
                                        }`}
                                />
                            </p>
                            <div
                                className={`animate-collapse grid common-transition ease-in-out ${showForm ? "grid-rows-[1fr] pt-2" : "grid-rows-[0fr] "
                                    }`}
                            >
                                <div className="overflow-hidden description text-sm md:text-base">

                                    <AddAddressForm mutate={mutate} setOpen={() => setShowForm(false)} />
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        {isValidating ? (
                            ...
                            data?.data?.data?.length === 0 ? (
                                ...
                                (
                                    data?.data?.data?.map((item: IAddress, i: number) => (
