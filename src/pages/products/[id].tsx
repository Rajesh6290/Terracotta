...
const Product = () => {
  ...
  const { data, isValidating, mutate } = useSwr(`product/getById/${router?.query?.id}`);
  ...
  return (
    <PublicLayout>
      <CustomerProductDetails item={data?.data?.data} isValidating={isValidating} mutate={mutate} />
      {
