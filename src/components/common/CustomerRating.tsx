...
const CustomerRatings = ({ open, close, productId, mutate, orderId }: { open: boolean; close: Dispatch<SetStateAction<boolean>>; productId: string; mutate: () => void; orderId: string }) => {
    ...
    const handleOperation = async (values: any, { resetForm }: FormikHelpers<any>) => {
        try {
            ...
            formData.append("star", String(value));
            formData.append("prodId", productId)
            files?.length > 0 && files?.map((item: any) => (
                formData.append("images", item)
            ))
            const res = await mutation(`rating`, {
                method: "POST",
                body: formData,
                isFormData: true,
                isAlert: true,
            ...
            if (res?.status === 200) {
                const response = await mutation(`order/rating/${orderId}`, {
                    method: "PUT",
                    isAlert: true,
                    body: {
                        prodId: productId,
    ...
    return (
        <Dialog open={open} onClose={() => { close(false); setFiles([]) }} maxWidth="xl">
