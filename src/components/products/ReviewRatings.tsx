...
const ReviewAndRating = ({ productId, rating, mutate }: { productId: string; rating: any; mutate: () => void }) => {
  ...
  const allReviewImages = rating?.rating?.map((item: any) => item?.images?.map((pre: any) => pre?.imageUrl));
  ...
  return (
    <>
      <AllImages open={imageOpen} close={() => setImageOpen(false)} item={allImages} />
      <ImagesAll open={imageOpenAll} close={() => setImageOpenAll(false)} item={allImages} />
      ...
      >
        ...
        <div className="w-full px-4 grid lg:flex grid-cols-2 gap-5 items-center">
          ...
          <div className="w-full grid lg:hidden grid-cols-2 items-center gap-5  relative">
            {
              finalImages && finalImages?.length > 2 && <span onClick={() => {
          ...
          <div className="w-full lg:grid hidden grid-cols-6 items-center gap-5  relative">
            {
              finalImages && finalImages?.length > 6 && <span onClick={() => {
                setImageOpenAll(true);
                setAllImages(finalImages)
            ...
            <div key={item._id} className="">
              <div className=" w-full flex flex-col gap-4 px-4 pb-4">
                ...
                <p className=" flex gap-1 items-center relative lg:hidden">
                  {
                    item?.images && item?.images?.length > 4 && <span onClick={() => {
                      setImageOpen(true);
                      setAllImages(item?.images)
                    }} className=" absolute top-1 right-0 w-14 h-14 bg-black rounded-md bg-opacity-70 cursor-pointer lg:hidden flex items-center justify-center text-white font-semibold tracking-wide text-xs ">View</span>
                  }
                ...
                <p className=" lg:flex hidden gap-1 items-center relative ">
                  ...
                  {
                    item?.images && item?.images?.length > 12 && <span onClick={() => {
                      setImageOpen(true);
                      setAllImages(item?.images)
...
const AllImages = ({ open, close, item }: any) => {
  return (
    <Dialog open={open} onClose={() => close(false)} maxWidth="xl" PaperProps={{
      style: {
        borderRadius: 18,
      },
    }}>
